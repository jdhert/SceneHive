...
const MEDIA_ENRICHMENT_CACHE_MAX_ENTRIES = Math.floor(
  readPositiveNumber(process.env.MEDIA_ENRICHMENT_CACHE_MAX_ENTRIES, 300)
);
const MEDIA_ENRICHMENT_CACHE_TIMEOUT_MS = readPositiveNumber(
  process.env.MEDIA_ENRICHMENT_CACHE_TIMEOUT_MS,
  20000
);
...
const mediaEnrichmentCache = createServerMemoryCache('media-enrichment', {
  maxEntries: MEDIA_ENRICHMENT_CACHE_MAX_ENTRIES,
});
...
export async function fetchTvDetailsPrimary(tvId: number) {
  return mediaEnrichmentCache.getOrLoad(
    ...
    () => loadTvDetailsPrimary(tvId),
    {
      ...
      timeoutMs: MEDIA_ENRICHMENT_CACHE_TIMEOUT_MS,
    }
  );
}
