...
3. `HANDOFF.md`의 최신 `Handoff Snapshot` 확인
4. 현재 작업의 제품 컨셉을 3줄 이내로 요약
5. 최신 `Handoff Snapshot` 기준으로 현재 작업 시작점 확인

권장 명령:
- `Get-Content -Path AGENTS.md, PROJECT_GUIDE.md, HANDOFF.md -Encoding UTF8`
...
아래 템플릿은 작업을 멈추기 전에 `HANDOFF.md`에 반드시 최신화한다.
...
1. `HANDOFF.md`의 최신 `Handoff Snapshot` 확인
2. 저장소 상태 확인
3. `Resume Command` 실행
...
3. `HANDOFF.md`의 `Handoff Snapshot` 갱신
4. 잔여 리스크 기록
...
- 체크포인트 기록(짧은 명령): `.\scripts\checkpoint.ps1` (`HANDOFF.md`에 기록)
- 체크포인트 로그 정리: `.\scripts\checkpoint-prune.ps1 -Keep 30` (`HANDOFF.md` 기준)
...
정기 정리 권장:
- 기본 정책: `HANDOFF.md`에 최신 `30`개 Snapshot 유지, 나머지는 `AGENTS_ARCHIVE.md`로 이동
...
- 최신/누적 `Handoff Snapshot`은 `HANDOFF.md`에 기록한다.
- `AGENTS.md`는 에이전트 운영 규칙과 프로젝트 고정 컨텍스트만 유지한다.
- 오래된 Snapshot은 `AGENTS_ARCHIVE.md`로 이동해 보관한다.
